...
//add table like and dislike in post table

function add_like_column($post_columns,$post_type){
      $columns = [
           'cb' => $post_columns['cb'],
           'title' => $post_columns['title'],
           'author' => $post_columns['author'],
           'categories' => $post_columns['categories'],
           'likes' => 'Likes',
           'dislikes' => 'Dislikes',
           'tags' => $post_columns['tags'],
           'date' => $post_columns['date'],
           'comments' => $post_columns['comments'],
      ];
     return $columns;
}
add_filter('manage_posts_columns','add_like_column',10,2);

function show_like_value($column_name,$post_id){
   if($column_name != 'likes'){
         return;
   }  
   $likes = (int) get_post_meta($post_id,'_post_likes');
   echo $likes;

}

add_action('manage_posts_custom_column','show_like_value',10,2);

function show_dislike_value($column_name,$post_id){
    if($column_name != 'dislikes'){
        return;
  }  
  $dislikes = (int) get_post_meta($post_id,'_post_dislikes');
  echo $dislikes;
}
add_action('manage_posts_custom_column','show_dislike_value',10,2);